...
@click.command()
@click.option("-y", "--no-input", help="Don't ask for any user input.", is_flag=True)
@click.option(
    "-s", "--smoothing", default=3.5, show_default=True, help="Smoothing amount."
)
@click.option(
    "-e",
    "--edge-preservation",
...
@click.argument(
    "input_image", type=click.Path(exists=True, file_okay=True, dir_okay=False)
)
